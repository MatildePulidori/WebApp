...
public class CourseController {
    ...
    @ResponseStatus(HttpStatus.CREATED)
    List<Boolean> entrollStudents(@PathVariable String name, @RequestParam("file") MultipartFile file) {
        try {
    ...
    @PostMapping("/{name}/proposeTeam/{teamId}")
    @ResponseStatus(HttpStatus.CREATED)
    TeamDTO proposeTeam(@PathVariable String name, @PathVariable String teamId, @RequestBody List<Map<String, String>> members) {
        List<String> studs = new ArrayList<>();
        members.stream().forEach( m  ->
                studs.add( m.get("id")) );

        TeamDTO teamDTO= teamServices.proposeTeam(name, teamId, studs);
